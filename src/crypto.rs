use bls12_381::{G1Affine, G2Affine, G1Projective, Scalar, pairing};
use bls12_381::hash_to_curve::{HashToCurve, ExpandMsgXmd, HashToField};
use rand::rngs::{OsRng};
use rand::RngCore;
...
mod tests {
    ...
    fn test_crypto_bls() {
        ...
        let Hmsg = G1Affine::from(g1_msg_proj);
        let sig : G1Affine = Hmsg.clone().mul(secret_key).into();

        let lhs = pairing(&sig, &g2);
        let rhs = pairing(&Hmsg, &public_key);
